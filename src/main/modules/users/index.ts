...
import { collectionName, Get, Exist, Distinct, FindOne, Insert, Update, Flag, Delete, setPersonNumber } from './actions'
ipcMain.handle(`${collectionName}.get`, async (event, args) => {
  try {
    const rs = { data: [], rowsNumber: 0, groups: [] }
    const conditions = { $and: [{ enable: args.enable ? args.enable : false }] } as any
    if (args.filter) {
      conditions.$and.push({
        $or: [
          { username: new RegExp(args.filter, 'i') },
          { fullName: new RegExp(args.filter, 'i') },
        ]
      })
    }
    if (args.group != '0') conditions.$and.push({ group: args.group })
    ...
    rs.groups = await Distinct('group')
...
ipcMain.handle(`${collectionName}.patch`, async (event, args) => {
  try {
    ...
    await session.withTransaction<any>(async () => {
...
ipcMain.handle(`${collectionName}.syncExt`, async (event, args) => {
  try {
    try {
      const rs = { s: [] as any, e: [] as any, ok: 0 }
      if (!args.personNumber) throw new Error('invalid')
      if (!Array.isArray(args.id)) throw new Error('invalid')
      const session = await startSession()
      await session.withTransaction<any>(async () => {
        for await (const id of args.id) {
          try {
            rs.ok = await setPersonNumber({ id: id, personNumber: args.personNumber }, session)
            if (!rs.ok) {
              rs.e.push(id)
              await session.abortTransaction()
              break
              ...
              rs.s.push(id)
              SetLog(collectionName, id, 'ext') // Set Log
            }
          } catch (e) {
            rs.e.push(id)
            await session.abortTransaction()
